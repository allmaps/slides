...
const imageBuilds = new Map<string, Promise<unknown>>();
const hash = (value: string | Uint8Array) => createHash("sha256").update(value).digest("hex");
const recipe = (source: string, options: ResolvedIiifOptions) => hash(JSON.stringify({
  generator: 2, sharp: sharp.versions, source, sizes: options.sizes,
  tiles: options.tiles, tileSize: options.tileSize, formats: options.outputFormats,
}));
async function createTiles(file: ImageFile, options: ResolvedIiifOptions): Promise<ImageServiceInfo> {
  // Serialize writes in one process; completed directories are published atomically.
  const previous = imageBuilds.get(file.outputDir) ?? Promise.resolve();
  const build = previous.catch(() => {}).then(async () => {
    const sourceHash = hash(await readFile(file.absolutePath));
    const identity = recipe(sourceHash, options);
    const expectedInfo = await getExpectedInfo(file, options);
    const marker = path.join(file.outputDir, ".source.json");
    let currentIdentity: string | undefined;
    try { currentIdentity = JSON.parse(await readFile(marker, "utf8")).identity; }
    catch (error) { if ((error as NodeJS.ErrnoException).code !== "ENOENT" && !(error instanceof SyntaxError)) throw error; }
    if (!options.force && currentIdentity === identity && await fileExists(file.infoPath)) {
      const currentInfo = await readInfoJson(file.infoPath);
      if (await isDerivativeCurrent(file, currentInfo, expectedInfo, options.outputFormats)) {
        console.log(`cached ${file.relativePath}`);
        return expectedInfo;
      }
    }
    const temporary = `${file.outputDir}.${randomUUID()}.tmp`;
    const staged = { ...file, outputDir: temporary, infoPath: path.join(temporary, "info.json") };
    try {
      await writeDerivatives(staged, expectedInfo, options);
      if (hash(await readFile(file.absolutePath)) !== sourceHash) throw new Error(`Image changed during generation; retry: ${file.absolutePath}`);
      await writeJson(path.join(temporary, ".source.json"), { identity });
      try {
        // Cold-cache publishers race through rename, never through deletion.
        await rename(temporary, file.outputDir);
      } catch (error) {
        if (!["EEXIST", "ENOTEMPTY"].includes((error as NodeJS.ErrnoException).code ?? "")) throw error;
        let existing: string | undefined;
        try { existing = JSON.parse(await readFile(marker, "utf8")).identity; } catch { /* incomplete cache entry */ }
        if (existing !== identity || options.force || !(await isDerivativeCurrent(file, expectedInfo, expectedInfo, options.outputFormats))) {
          await rm(file.outputDir, { recursive: true, force: true });
          await rename(temporary, file.outputDir);
        }
      }
      console.log(`render ${file.relativePath}`);
      return expectedInfo;
    } finally { await rm(temporary, { recursive: true, force: true }); }
  });
  imageBuilds.set(file.outputDir, build);
  try { return await build; }
  finally { if (imageBuilds.get(file.outputDir) === build) imageBuilds.delete(file.outputDir); }
}
...
export type BuildStaticIiifSettings = {
  ...
  /** Private derivative cache; public filenames are recorded separately. */
  cacheRoot?: string;
};

export type BuildStaticIiifResult = {
  assets: Record<string, { filename: string; type: string }>;
  files: string[];
  images: ProcessedImage[];
  options: ResolvedIiifOptions;
};
...
export async function buildStaticIiif(
  options: ResolvedIiifOptions,
  { failOnEmpty = true, images, cacheRoot }: BuildStaticIiifSettings = {},
  ...
  const assets: BuildStaticIiifResult["assets"] = {};
  const addOutputFile = (filePath: string, filename = filePath) => {
    const request = path.relative(options.outputRoot, filePath).split(path.sep).join("/");
    outputFiles.add(request);
    assets[request] = { filename, type: getContentType(filePath) ?? "application/octet-stream" };
  };

  const processedImages: ProcessedImage[] = [];
  for (const file of files) {
    const identity = cacheRoot ? recipe(hash(await readFile(file.absolutePath)), options) : undefined;
    const cachedDir = identity ? path.join(cacheRoot!, "images", identity) : file.outputDir;
    const cachedFile = { ...file, outputDir: cachedDir, infoPath: path.join(cachedDir, "info.json") };
    const info = await createTiles(cachedFile, options);
    // IDs belong to the publication; immutable cached pixels do not depend on its URL.
    await writeJson(file.infoPath, info);
    addOutputFile(file.infoPath);
    const derivatives = getExpectedDerivativePaths(file, info, options.outputFormats);
    const cached = getExpectedDerivativePaths(cachedFile, info, options.outputFormats);
    derivatives.forEach((filename, index) => addOutputFile(filename, cached[index]));
...
const unwrapImageModule = (mod: IiifImageModule) => {
  ...
  if (!isIiifImageSource(source)) {
    throw new Error(
      "Invalid IIIF image import. Make sure @allmaps/iiif/vite is enabled before Vite's asset plugin.",
    );
  }
...
export function createIiifCatalog(
  ...
  const ensureBuilt = (publicUrl?: string) => {
    ...
    if (!buildPromise) {
      ...
      buildPromise.catch(() => { if (buildPromises.get(buildKey) === buildPromise) buildPromises.delete(buildKey); });
  ...
  return {
    ...
    get: async (request, requestOptions) => {
      const result = await ensureBuilt(requestOptions?.publicUrl);
      const filePath = request && Object.hasOwn(result.assets, request) ? result.assets[request].filename : undefined;
