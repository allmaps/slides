...
const resolveContentImage = (assetPath: string): ImageModule | undefined =>
  imageAssetUrls[`./${assetPath}`];

const isContentDataAssetPath = (path: string) =>
  path.startsWith("assets/") && DATA_ASSET_EXTENSION_PATTERN.test(path);

const getContentDataAssetRequestPath = (assetPath: string) =>
...
export const getContentDataAssetUrl = (
  ...
  return withBaseUrl(
    joinUrl("api", getContentDataAssetRequestPath(assetPath)),
  );
};
...
export const getContentAssetUrl = (
  ...
  const assetPath = normalizeProjectAssetPath(cleanPath);
  const dataAssetUrl = getContentDataAssetUrl(assetPath);
  if (dataAssetUrl) return dataAssetUrl;

  const image = resolveContentImage(assetPath);
